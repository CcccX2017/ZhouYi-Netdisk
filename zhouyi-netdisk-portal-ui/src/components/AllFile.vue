<template>
    <div>
        <div class="file-fun-area">
            <div class="fun-btn">
                ...
                <el-button-group class="btn-group" v-if="btnGroup.show">
                    ...
                    <el-button type="primary" plain class="plain-btn" :disabled="btnGroup.disabled"
                               @click.native.stop="openRenameDialog">重命名
                    </el-button>
						...
						<li>
							...
							<template v-for="(item, index) in breadcrumb">
								<el-link type="primary" v-if="index != breadcrumb.length - 1 && item.name != '...'"
                                         @click="goBack(item.path)" :title="item.name">
									{{ item.name | breadcrumbFilter(breadcrumb.length) }}
								</el-link>
								<span class="breadcrumbTxtSpan" v-else
                                      :title="item.name">{{ item.name | breadcrumbFilter(breadcrumb.length) }}</span>
                ...
                <div id="fileDiv" class="list-content" :style="content">
                    <vue-scroll ref="vs" style="top: -1px;border: none;" @handle-scroll="handleScroll">
                        <template v-for="(item, index) in list">
                            <div class="content-row clearfix" :key="index + new Date().getTime()"
                                 @click.prevent="checkOne(item)" :class="index == 0 ? 'first' : ''">
                                ...
                                <template v-else>
                                    ...
                                    <div class="left content-col" style="width: 10%">
										<span class="txtSpan">
											<a href="javascript:;" class="dirLink" :title="assembleDirectory(item.dir)"
        ...
        <el-dialog title="新建文件夹" :visible.sync="dialogFolderVisible" width="350px" custom-class="folderDialog"
            ...
            <el-form :model="folderForm" :rules="folderRules" ref="folderForm">
                <el-form-item prop="folderName">
                    <el-input v-model="folderForm.folderName" placeholder="请输入文件夹名称" @keyup.enter.native="createFolder"
                              ref="folderInput"></el-input>
                </el-form-item>
            </el-form>
        ...
        <!-- 重命名 -->
        <el-dialog title="重命名" :visible.sync="dialogRenameVisible" width="350px" custom-class="folderDialog"
                   :close-on-click-modal="false">
            <el-form :model="renameForm" :rules="renameRules" ref="renameForm">
                <el-form-item prop="fileName">
                    <el-input v-model="renameForm.fileName" placeholder="请输入文件(夹)名称" @keyup.enter.native="rename"
                              ref="renameInput"></el-input>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
